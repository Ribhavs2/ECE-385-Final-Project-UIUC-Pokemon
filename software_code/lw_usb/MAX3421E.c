...
void MAXreg_wr(BYTE reg, BYTE val) {
    ...
    ret_code = XSpi_Transfer(&SpiInstance, writeBuffer, NULL, sizeof(writeBuffer));

	if (ret_code != 0) {
        xil_printf("Error in SPI transfer: %d\n", ret_code);
    }

	XSpi_SetSlaveSelect(&SpiInstance, 0);

	// XGpio_DiscreteWrite(&Gpio_int, 1, SLV_SSON);
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	ret_code = XSpi_Transfer(&SpiInstance, writeBuffer, readBuffer, sizeof(writeBuffer));
	//read return code from SPI peripheral 
	//if return code != 0 print an error
	if (ret_code != 0) {
        xil_printf("Error in SPI transfer: %d\n", ret_code);
