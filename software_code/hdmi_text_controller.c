...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue)
{
	...
	if (color %2 == 0) {
		blue_shift = ((uint32_t) blue) << 1;
		green_shift = ((uint32_t) green) << 5;
		red_shift = ((uint32_t) red) << 9;

		hdmi_ctrl -> palette[color >> 1] &= 0xFFFFE000;
		hdmi_ctrl -> palette[color >> 1] += blue_shift + green_shift + red_shift;
	}
	else {
		blue_shift = ((uint32_t) blue) << 13;
		green_shift = ((uint32_t) green) << 17;
		red_shift = ((uint32_t) red) << 21;

		hdmi_ctrl -> palette[color >> 1] &= 0x00001FFF;
		hdmi_ctrl -> palette[color >> 1] += blue_shift + green_shift + red_shift;
	}
}
...
void textHDMIColorScreenSaver()
{
	...
	while (1)
	{
		...
		xil_printf(color_string, "Drawing %s text with %s background", colors[fg].name, colors[bg].name);
